...
namespace solexp.Controllers
{
    ...
    public class TeacherController : ControllerBase
    {
        ...
        /// </summary>
        [HttpPut("lessons/{lessonId}/status")]
        public async Task<ActionResult> UpdateStatus(int lessonId, [FromBody] string status)
        {
            try
            {
                var lesson = await _teacherService.GetLessonDetailsAsync(lessonId);
                ...
                await _teacherService.UpdateLessonStatusAsync(lessonId, status);
                return Ok(new { message = "Статус занятия обновлён" });
            }
            catch (ArgumentException ex)
            {
                return BadRequest(ex.Message);
            ...
            {
                _logger.LogError(ex, "Ошибка при обновлении статуса занятия");
                return StatusCode(500, "Внутренняя ошибка сервера");
            }
        }
        ...
        public async Task<ActionResult> UpdateEmail([FromBody] UpdateEmailDto dto)
        {
            ...
            {
                var teacherId = GetCurrentTeacherId();
                await _teacherService.UpdateEmailAsync(teacherId, dto.NewEmail, dto.CurrentPassword);
        ...
        public async Task<ActionResult> UpdatePassword([FromBody] UpdatePasswordDto dto)
        {
            ...
            {
                var teacherId = GetCurrentTeacherId();
                await _teacherService.UpdatePasswordAsync(teacherId, dto.CurrentPassword, dto.NewPassword);
